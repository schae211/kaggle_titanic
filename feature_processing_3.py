...
def load_and_process_data_set():
    ...
    # Replacing embarked with numerical values in train and test set
    replace_by_map(df_train, "Embarked", {"S": 0, "C": 1, "Q": 2})
    replace_by_map(df_test, "Embarked", {"S": 0, "C": 1, "Q": 2})
...
def NaN_processing(df):
    ...
    df["Embarked"] = df["Embarked"].fillna("S")         # Replace NaN in Embarked column with S
